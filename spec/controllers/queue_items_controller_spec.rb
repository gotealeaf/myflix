...
describe QueueItemsController do
  ...
  describe "POST update_queue" do
    context "with valid inputs" do
      let(:darren) { Fabricate(:user) }
      let(:video) { Fabricate(:video) }
      let(:queue_item1) { Fabricate(:queue_item, user: darren, position: 1, video: video) }
      let(:queue_item2) { Fabricate(:queue_item, user: darren, position: 2, video: video) }
      before do
        session[:user_id] = darren.id
      end
      it "redirects to the my_queue page" do
