require 'spec_helper'
  describe QueueItem do
    ...
    describe "#rating=" do
      it "changes the rating of the reveiw if the review is present" do
        video = Fabricate(:video)
        user = Fabricate(:user)
        review = Fabricate(:review, user: user, video: video, rating: 2)
        queue_item = Fabricate(:queue_item, user: user, video: video)
        queue_item.rating = 4
        expect(Review.first.rating).to eql(4)
      end
      it "clears the rating of the reveiw if the reveiw is present" do
        video = Fabricate(:video)
        user = Fabricate(:user)
        review = Fabricate(:review, user: user, video: video, rating: 2)
        queue_item = Fabricate(:queue_item, user: user, video: video)
      ...
      it "creates a review with the rating if the review is not present" do 
        ...
        queue_item = Fabricate(:queue_item, user: user, video: video)
        queue_item.rating = 3
        expect(Review.first.rating).to eql(3)
      end
    end
