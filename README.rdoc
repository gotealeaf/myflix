...
2. Test environment setup procedures:
* Ruby version 2.0
* Run ```$ bundle install``` to install gem dependencies.
* Run ```$ rake db:migrate db:test:prepare``` and ```$ rake db:seed``` to set up development database.
...
4. Running Server:
* Run Redis-server with ```$ src/redis-server``` in seperate shell
* Run ```$ foreman start``` in seperate shell
* Run ```$ rails s``` in seperate shell
